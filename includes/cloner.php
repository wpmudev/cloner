...
class WPMUDEV_Cloner {
	...
	public function add_site_action_link( $links, $blog_id ) {
		...
		if ( ! is_main_site( $blog_id ) && $blog_id !== 1 )
			$links['clone'] = '<span class="clone"><a href="' . $clone_url . '">' . __( 'Clone', 'wpmudev-cloner' ) . '</a></span>';
	...
	public function render_admin_menu() {
		...
		if ( isset( $_REQUEST['cloned'] ) ) {
			...
			$messages = array(
				sprintf( __( 'Your new site has been cloned. <a href="%s">Go to dashboard</a>', 'wpmudev-cloner' ), esc_url( get_admin_url( $_REQUEST['cloned'] ) ) )
			);
		}
		...
		if ( ! empty( $_REQUEST['clone-site-submit'] ) ) {
		    ...
		    if ( ! empty( $blog_details ) ) {
		        ...
		        if ( $blog_details->blog_id === 1 || is_main_site( $blog_details->blog_id ) )
		            wp_die( __( 'Sorry, main site cannot be overwritten', 'wpmudev-cloner' ) );

		        if ( $blog_details->blog_id == $blog_id )
		            wp_die( __( 'You cannot clone a blog to its own domain', 'wpmudev-cloner' ) );
